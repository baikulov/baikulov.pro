...
async def send_to_telegram(bot, title, description, link, tags):
    ...
    image_url = urljoin('https://baikulov.pro/blog/_attachments/', image_tag.get('src').split("/")[-1]) if image_tag else None
    ...
    try:
        if image_url:
            ...
            send_function = bot.send_photo if image_url.startswith('https://baikulov.pro/blog/') else bot.send_message
